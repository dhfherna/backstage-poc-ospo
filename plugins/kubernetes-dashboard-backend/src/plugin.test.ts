...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [kubernetesDashboardPlugin],
    });

    await request(server).get('/api/kubernetes-dashboard/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/kubernetes-dashboard/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/kubernetes-dashboard/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/kubernetes-dashboard/todos/${createdTodoItem.id}`)
